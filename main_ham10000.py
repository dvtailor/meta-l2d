...
from lib.datasets import ContextSampler, load_ham10000
    ...
    with torch.no_grad():
        ...
        for data in data_loader:
            ...
            if config["l2d"] == 'pop':
                ...
                exp_preds = torch.tensor(expert(expert_cntx.xc.squeeze(0), expert_cntx.yc.squeeze(), cntx_yc_sparse),
                                         device=device)
...
def eval(model, test_data, loss_fn, experts_test, cntx_sampler, config):
    model.load_state_dict(
        torch.load(os.path.join(config["ckp_dir"], config["experiment_name"] + ".pt"), map_location=device))
    model = model.to(device)
    kwargs = {'num_workers': 0, 'pin_memory': True}
...
def main(config):
    ...
    n_features = model.classifier[0].out_features

    if config["l2d"] == "pop":
        model = ClassifierRejectorWithContextEmbedder(model, num_classes=int(config["n_classes"]),
                                                      n_features=n_features,
                                                      with_cross_attn=with_cross_attn, with_self_attn=with_self_attn,
                                                      with_softmax=with_softmax)
    else:
        model = ClassifierRejector(model, num_classes=int(config["n_classes"]), n_features=n_features,
                                   with_softmax=with_softmax)
    ...
    for _ in range(config["n_experts"]):  # train
        ...
        expert = SyntheticExpertOverlap(class_oracle=class_oracle, n_classes=config["n_classes"], p_in=1.0,
                                        p_out=config['p_out'])
    ...
    test_data_cntx, test_data_trgt = torch.utils.data.random_split(test_data,
                                                                   [test_cntx_size, len(test_data) - test_cntx_size], \
                                                                   generator=torch.Generator().manual_seed(
                                                                       config["seed"]))
